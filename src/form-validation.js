...
import { useRef } from "react-hook-form";
    ...
    const onSubmitForm = (event) => {
        ...
        for(let f of inputFile.current.files){
            if(f.size > maxSize * 1000){
                alert(`ไม่เกิน 3 ไฟล์ ขนาดสูงสุด 300kB/ไฟล์`)
                event.preventDefault()
                return
            }
        }
        ...
        <div className = "mt-3 mb-5 mx-auto p-3 rounded"
            style = {{width:'400px',background:'#cee'}}>
                ...
                <form onSubmit={handleSubmit(onSubmitForm)}>
                    ...
                    <div className ="form-group mb-3">
                        <label htmlFor="datail">รายละเอียดสินค้า *</label>
                        <textarea id ="datail" name="datail" rows="2" maxLength="250"
                        className = "form-control form-control-sm"
                        {...register('datail',{required:true,minLength:1,maxLength:200})}></textarea>
                        {errors.datail && <div style={err}>ต้องกำหนดรายละเอียดสินค้า</div>}
                    </div>
                    <div className="form-group mb-3">
                    ...
                    <div className="form-group mb-3">
                        ...
                        <input type ="file" id="file" accept="image/*"
                            className="form-control form-cotrol-sm" ref={inputFile}multiple/>
                    </div>
